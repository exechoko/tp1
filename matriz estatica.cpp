...
int main()
{
    ...
    for(i=0;i<filas;i++){
        for(j=0;j<columnas;j++){
            ...
            if (( M[i][j] != M[i][j+1] ) && ( M[i][j] != M[i+1][j] ) && ( M[i][j] != M[i][j-1] ) && ( i == 0) && (M[i][j] ==1)) // compara en la primer fila i=0
                cout << " Elemento: [" << i << "][" << j << "] " << "invalido  i: " << i << " j: " << j << endl;
            else

            if (( M[i][j] != M[i][j+1] ) && ( M[i][j] != M[i+1][j] ) && ( M[i][j] != M[i-1][j] ) && ( j == 0) && (M[i][j] ==1) ) // compara en la primer columna
                cout << " Elemento: [" << i << "][" << j << "] " << "invalido  i: " << i << " j: " << j << endl;
            else

            if (( M[i][j] != M[i][j+1] ) && ( M[i][j] != M[i+1][j] ) && ( M[i][j] != M[i-1][j] ) && ( M[i][j] != M[i][j-1] ) && ( i != 0) &&( j != 0) && (M[i][j] ==1)) // compara en el resto de la matriz
                cout << " Elemento: [" << i << "][" << j << "] " << "invalido  i: " << i << " j: " << j << endl;
